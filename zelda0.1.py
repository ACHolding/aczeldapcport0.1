#!/usr/bin/env python3
"""
AC's Tiny Cap Adventure 0.1
================================
A single-file, original GBA-style top-down action-adventure for Python 3.14
...
Run:
    py -3.14 ac_tiny_cap_adventure.py
...
FPS = 60
TILE = 16
TITLE = "AC's Tiny Cap Adventure 0.1 — GBA Speed"
...
class Enemy:
    ...
    @property
    def rect(self) -> pygame.Rect:
        size = 18 if self.kind == "boss" else 11
    ...
    def take_damage(self, amount: int, source: tuple[float, float], game: "Game") -> None:
        ...
        if away.length_squared():
            ...
            self.y += away.y
        game.audio.play("hit")
        game.shake = 2 if self.kind != "boss" else 4
    ...
    def update(self, dt: float, game: "Game", room: Room) -> None:
        ...
        near = target.length_squared() < (84 if self.kind != "boss" else 120) ** 2
        if near and target.length_squared():
            target = target.normalize()
            if self.kind == "bat":
                target.rotate_ip(math.sin(self.phase * 1.7) * 35)
            if self.kind == "boss" and math.sin(self.phase) > 0.72:
                target *= 2.4
...
def build_rooms() -> dict[str, Room]:
    ...
    return {room.name: room for room in (village, forest, dungeon_entry, dungeon_hall, boss)}
...
class Game:
    ...
    def on_enemy_defeated(self, enemy: Enemy) -> None:
        if enemy.kind == "boss":
            ...
            self.open_dialogue(
                "Sanctum",
                [
                    "The Emberheart fades. The forest breathes again.",
                    "Approach the Sunbud altar to complete your quest.",
                ],
            )
            return
    ...
    def interact(self) -> None:
        ...
        for npc in self.room.npcs:
            if distance(center, npc.rect.center) <= 25:
                if npc.name == "Elder Fern":
                    if "boss_defeated" in self.flags:
                        lines = ["You returned with dawn in your wake. Mossbell remembers."]
                    elif self.shards >= 3:
        ...
        if self.room_name == "dungeon_entry":
            chest = pygame.Rect(5 * TILE, 4 * TILE, TILE, TILE)
            if distance(center, chest.center) <= 24:
                if not self.room.chest_open:
                    ...
                    self.open_dialogue(
                        "Treasure Chest",
                        ["You found the Sun Key! It opens a brilliant lock."],
                    )
                else:
                    self.open_dialogue("Treasure Chest", ["The chest is empty."])
                return

        if self.room_name == "boss" and "boss_defeated" in self.flags:
    ...
    def find_prompt(self) -> str:
        ...
        if self.room_name == "forest":
            if distance(center, (12 * TILE + 8, 6 * TILE + 8)) <= 25:
                return "E Use stone"
        if self.room_name == "dungeon_entry":
            if distance(center, (5 * TILE + 8, 4 * TILE + 8)) <= 24:
                return "E Open"
        if self.room_name == "boss" and "boss_defeated" in self.flags:
    ...
    def draw_tile(self, surface: pygame.Surface, char: str, tx: int, ty: int) -> None:
        ...
        if self.room.theme == "outdoor":
            base = COLORS["grass"] if seed % 3 else COLORS["grass2"]
            ...
            if char == "=":
                ...
                pygame.draw.circle(surface, (27, 107, 59), (x + 8, y + 7), 8)
                pygame.draw.circle(surface, (57, 151, 70), (x + 5, y + 4), 4)
                ...
                pygame.draw.rect(surface, (104, 53, 45), (x, y, TILE, 5))
                pygame.draw.line(surface, (222, 149, 82), (x, y + 8), (x + 15, y + 8))
            elif char == "S":
    ...
    def draw_hud(self, surface: pygame.Surface) -> None:
        panel = pygame.Surface((INTERNAL_W, 15), pygame.SRCALPHA)
        panel.fill((14, 18, 28, 215))
        ...
        self.draw_text(surface, f"$ {self.rupees:02}", (52, 3), (81, 230, 122), self.font_small)
        self.draw_text(surface, f"Mote {self.shards}/3", (82, 3), COLORS["gold"], self.font_small)
        if self.has_key:
            self.draw_text(surface, "KEY", (131, 3), COLORS["cream"], self.font_small)
        mode = "TINY" if self.player.tiny else "HERO"
        self.draw_text(surface, mode, (207, 3), (126, 223, 255), self.font_small)
        if self.prompt and not self.dialogue:
            width = self.font.size(self.prompt)[0] + 8
            pygame.draw.rect(surface, (18, 23, 35), (INTERNAL_W - width - 4, 18, width, 13))
            pygame.draw.rect(surface, COLORS["gold"], (INTERNAL_W - width - 4, 18, width, 13), 1)
            self.draw_text(surface, self.prompt, (INTERNAL_W - width, 21), COLORS["cream"])

    def objective(self) -> str:
        if "boss_defeated" in self.flags:
            return "Touch the Sunbud altar."
        if self.room_name == "boss":
            return "Defeat the Emberheart."
        if self.has_key:
            return "Find the Sun-lock."
        if self.room_name.startswith("dungeon"):
    ...
    def draw_dialogue(self, surface: pygame.Surface) -> None:
        box = pygame.Rect(7, 113, 226, 40)
        pygame.draw.rect(surface, (20, 25, 39), box)
        pygame.draw.rect(surface, COLORS["cream"], box, 2)
        pygame.draw.rect(surface, COLORS["gold"], (12, 108, 72, 12))
        self.draw_text(surface, self.dialogue_speaker, (16, 110), COLORS["ink"])
        ...
        for word in words:
            ...
            if self.font.size(test)[0] > 208 and current:
        ...
        for index, text in enumerate(lines[:3]):
            self.draw_text(surface, text, (14, 124 + index * 9))
        self.draw_text(surface, "E", (220, 143), COLORS["gold"])
    ...
    def render_world(self) -> None:
        ...
        boss = next((enemy for enemy in self.room.enemies if enemy.kind == "boss" and enemy.alive), None)
        if boss:
            ...
            self.draw_text(self.canvas, "EMBERHEART", (90, 26), COLORS["cream"], self.font_small)
    ...
    def render_title(self) -> None:
        self.canvas.fill((31, 82, 64))
        for y in range(0, INTERNAL_H, 16):
            for x in range(0, INTERNAL_W, 16):
                color = (38, 102, 71) if (x // 16 + y // 16) % 2 else (34, 93, 67)
        ...
        pygame.draw.ellipse(self.canvas, (21, 57, 50), (62, 28, 116, 87))
        hero = pygame.transform.scale(self.assets[112], (48, 48))
        self.canvas.blit(hero, hero.get_rect(center=(120, 69)))
        pygame.draw.rect(self.canvas, (17, 23, 35), (12, 107, 216, 47))
        pygame.draw.rect(self.canvas, COLORS["gold"], (12, 107, 216, 47), 2)
        self.draw_text(self.canvas, "AC'S", (120, 10), COLORS["gold"], self.font_big, True)
        self.draw_text(self.canvas, "TINY CAP ADVENTURE", (120, 27), COLORS["cream"], center=True)
        blink = COLORS["gold"] if int(self.title_time * 2) % 2 else COLORS["cream"]
        self.draw_text(self.canvas, "ENTER  START", (120, 118), blink, center=True)
        self.draw_text(self.canvas, "Z Sword  X Shield  Shift Roll  E Use", (120, 135), COLORS["cream"], self.font_small, True)
        self.draw_text(self.canvas, "240x160 • 60 FPS • CC0 ART", (120, 146), (137, 205, 213), self.font_small, True)
    ...
    def render_end_screen(self, victory: bool) -> None:
        ...
        if victory:
            ...
            sub = "The forest shines. Tiny hero, giant courage."
        else:
...
def run_smoke_test() -> None:
    ...
    pygame.quit()
    print("Smoke test passed: 5 rooms, embedded assets, render/update loop.")
